...
function test()
{
    echo anchor('site_cookies/test_set','Set Cookie');
    echo '<hr>';
    echo anchor('site_cookies/test_destroy', 'Destroy Cookie');
    
    $user_id = $this->_attempt_get_user_id();

    if (is_numeric($user_id)) {
        echo '<h1>Yor are user '.$user_id.'</h1>';
    }  
}
...
function test_set()
{
    ...
    echo anchor('site_cookies/test','Get Cookie');
    echo '<hr>';
    echo anchor('site_cookies/test_destroy', 'Destroy Cookie');
}
...
{
    // check this user has a valid cookie and if so figure out cookie_name
    $this->load->module('site_settings');
    $cookie_name = $this->site_settings->_get_cookie_name();
